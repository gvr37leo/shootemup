...
function createRandomShotgunBlastAbility(pos:Vector,gamedb:GameDB){
    ...
    return new Ability(() => {
        for(var i = 0; i < amount; i++){
            gamedb.friendlyBullets.push(new Bullet(pos.c(),rotate2d(new Vector(0,1),randomSpread(0,spread)).scale(random(minspeed,maxspeed))))
        }
    })
}
...
function bulletRingSpawn(begin:number,step:number,numberOfSteps:number,origin:Vector,speed):Bullet[]{
    ...
    for(var i = 0; i < numberOfSteps; i++){
        var dir = rotate2d(new Vector(0,-1),(begin + i * step)).scale(speed)
